...
class room_process:
                    ...
                    for r in range(1, self.total_rows):
                        if math.isnan(self.df.iloc[r,c]) and self.df.iloc[r-1,c] < float('inf') and flag == 0:
                            ...
                            for i in range(1, r_count):
                                self.df.iloc[r_start+i, c] = self.df.iloc[r_start, c] + i*dif_each
                    else:
                        continue
    ...
    def write_csv_results(self):
        self.df.to_csv(self.dir_path + '/' + self.name[0:6] + '_result.csv')
...
class heatpump_process:
    ...
    def write_csv_results(self):
        self.df.to_csv(self.dir_path + '/' + self.name[0:10] + '_result.csv')

...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            if io.temp2 > 0:
                # 若成功，写入数据库
                db_operate(cursor, 'sensor_room', db_time, '0x00000222', 'room_temp2', str(io.temp2), step)
            else:
                # 若失败，写入报错信息
            ...
            if io.rh2 > 0:
                # 若成功，写入数据库
                db_operate(cursor, 'sensor_room', db_time, '0x00000223', 'room_RH2', str(io.rh2), step)
            else:
                # 若失败，写入报错信息
            ...
            #fan_state = fan_read_modify(5, fcu.fan)                      # Need configuration for each node!
            #print("fan_feecback: " + str(fan_state))
            if fcu.fan in [0,1,2,3]:
                db_operate(cursor, 'fcu_panel', db_time, '0x00000412', 'FCU_fan_feedback', str(fcu.fan), step)
            else:

...
def taskFunction(self,id,adjDirection,datalist,tasknum):
    ...
    if type == 2:  #pump_1
        ...
        self.sendUDP("G_s: "+str(G_s),tasknum)
        ...
        while (1):
            ...
            if (step_min - step_rec) % T_inter_Q == 0:
                ...
                # deltaT = cal_deltaT(db)
                G_s = Qtotal / (4.2 * 1000 * delta_set) * 60  / T_inter_con   # m3/h
                self.sendUDP("time: "+str(step_min)+" min; G_s: "+str(G_s),tasknum)
                start_time_Q = time.time()
                db.commit()
            if (step_min - step_rec) % T_inter_con == 0:
                G_k, speed_k = read_pump(db)
                speed_s, deltaSpeed_last = con_pump(G_s,G_k,speed_k,deltaSpeed_last)
                write_controlpump(db, step_min, speed_s)
                start_time_con = time.time()
                db.commit()
            step_min += 1
